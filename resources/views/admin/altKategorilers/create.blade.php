...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.altKategoriler.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.alt-kategorilers.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="subcategory_name">{{ trans('cruds.altKategoriler.fields.subcategory_name') }}</label>
                <input class="form-control {{ $errors->has('subcategory_name') ? 'is-invalid' : '' }}" type="text" name="subcategory_name" id="subcategory_name" value="{{ old('subcategory_name', '') }}" required>
