    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.deliveries.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('delivery_date') ? 'is-invalid' : '' }}" type="text" name="delivery_date" id="delivery_date" value="{{ old('delivery_date') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('delivery_employee') ? 'is-invalid' : '' }}" type="text" name="delivery_employee" id="delivery_employee" value="{{ old('delivery_employee', '') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('plaka') ? 'is-invalid' : '' }}" name="plaka" id="plaka" required>
                    ...
                    @foreach(App\Models\Delivery::PLAKA_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('plaka', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('plaka'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('seal_number') ? 'is-invalid' : '' }}" type="text" name="seal_number" id="seal_number" value="{{ old('seal_number', '') }}" required>
