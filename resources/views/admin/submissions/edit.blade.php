    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.submissions.update", [$submission->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('store_delivery_date') ? 'is-invalid' : '' }}" type="text" name="store_delivery_date" id="store_delivery_date" value="{{ old('store_delivery_date', $submission->store_delivery_date) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('story_delivery_employee') ? 'is-invalid' : '' }}" name="story_delivery_employee" id="story_delivery_employee">
                    ...
                    @foreach(App\Models\Submission::STORY_DELIVERY_EMPLOYEE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('story_delivery_employee', $submission->story_delivery_employee) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('deliveryid') ? 'is-invalid' : '' }}" name="deliveryid_id" id="deliveryid_id" required>
                    @foreach($deliveryids as $id => $entry)
                        <option value="{{ $id }}" {{ (old('deliveryid_id') ? old('deliveryid_id') : $submission->deliveryid->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('depoid') ? 'is-invalid' : '' }}" name="depoid_id" id="depoid_id" required>
                    @foreach($depoids as $id => $entry)
                        <option value="{{ $id }}" {{ (old('depoid_id') ? old('depoid_id') : $submission->depoid->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
